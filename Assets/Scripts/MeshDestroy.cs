...
public class MeshDestroy : MonoBehaviour
{
    ...
    public void DestroyMesh()
    {
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            parts[i].MakeGameobject(this, parts[i]);
    //     if (parts[i].NewEmpty.GetComponent<Rigidbody>()) parts[i].NewEmpty.GetComponent<Rigidbody>().AddForceAtPosition(parts[i].Bounds.center * ExplodeForce, transform.position);
          //parts[i].NewEmpty.AddComponent<Rigidbody>().AddForceAtPosition(parts[i].Bounds.center * ExplodeForce, transform.position);
        }
    ...
    public class PartMesh
    {
        ...
        public void MakeGameobject(MeshDestroy original, PartMesh part)
        {
            NewEmpty = original.NewEmpty;
           // GameObject = PhotonNetwork.Instantiate(new GameObject().name, original.transform.position, original.transform.rotation);
            GameObject empty = PhotonNetwork.Instantiate(NewEmpty.name, original.transform.position, original.transform.rotation);
            //GameObject =  PhotonNetwork.Instantiate( new GameObject(original.name));
            empty.transform.localScale = original.transform.localScale;
            ...
            // var renderer = empty.AddComponent<MeshRenderer>();
            empty.GetComponent<MeshRenderer>().materials = original.GetComponent<MeshRenderer>().materials;
            empty.GetComponent<MeshFilter>().mesh = mesh;

            //var filter = empty.AddComponent<MeshFilter>();
            //filter.mesh = mesh;
            empty.GetComponent<MeshCollider>().convex = true;
          //  var collider = empty.AddComponent<MeshCollider>();
           // collider.convex = true;

            //var rigidbody = empty.AddComponent<Rigidbody>();
            empty.GetComponent<Rigidbody>().useGravity = true;
           empty.GetComponent<Rigidbody>().AddForceAtPosition(part.Bounds.center * original.ExplodeForce, original.transform.position);
          //  var Photon = GameObject.AddComponent<PhotonView>();
         /*   var PhotonTransform = GameObject.AddComponent<PhotonTransformView>();
            ...
            PhotonTransform.m_UseLocal = true;*/
            empty.AddComponent<DestroySmth>();
            MeshDestroy meshDestroy = empty.AddComponent<MeshDestroy>();

            meshDestroy.NewEmpty = NewEmpty;
